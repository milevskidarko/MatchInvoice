...
function parseInvoiceData(text: string) {
  ...
  for (const line of lines) {
    ...
    if (itemMatch) {
      ...
      let vat = globalVat !== undefined ? globalVat : 18;
      items.push({
        name,
        qty: isNaN(qty) ? 1 : qty,
...
export async function POST(request: Request) {
  ...
  try {
    if (file.type === "application/pdf") {
      ...
      // Support both ESM and CJS/named export styles
      const getDocument = pdfjsLib.getDocument || (pdfjsLib.default && pdfjsLib.default.getDocument);
      if (!getDocument) {
        throw new Error("pdfjs-dist getDocument not found. Check pdfjs-dist version and import style.");
      }
